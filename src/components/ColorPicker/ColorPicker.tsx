...
export class VColorPicker extends Component<IProps, IState> {
  ...
  hexToRgb = (hex: string) => {
    ...
    let color = /^#?([a-f\d]{2})([a-f\d]{2})([a-f\d]{2})$/i.exec('#ff0000');
    if (isHex) {
      color = /^#?([a-f\d]{2})([a-f\d]{2})([a-f\d]{2})$/i.exec(hex);
    } else {
      if (typeof this.getNameColors()[hex.toLowerCase()] !== 'undefined')
        color = /^#?([a-f\d]{2})([a-f\d]{2})([a-f\d]{2})$/i.exec(this.getNameColors()[hex.toLowerCase()]);
    }
